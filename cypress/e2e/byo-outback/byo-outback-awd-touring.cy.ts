...
const variantColourCount: number = outbackPageElements.colorOptions.awdTouring.length
const variantColourOptions = outbackPageElements.colorOptions.awdTouring
const variantInteriorCount: number = outbackPageElements.interiorOptions.awdTouring.length
const variantStylingPackCount: number = 6
const variantInteriorOptions = outbackPageElements.interiorOptions.awdTouring
const modelCode = 'AUOUT'
const variantCode = 'AUOW25TA'
...
let interiorZeroAmountIndex = $PageObject.getRandomNumber(variantInteriorCount-2);
let optionsIndex = $PageObject.getRandomNumber(variantStylingPackCount-1);

describe("Build Your Own - Outback AWD Touring", () => {
  ...
  it("CY_08 - Ability to select any variant color", () => {
    ...
    $PageObject.validateCarImgSrc(outbackPageElements.imgSrc.awdTouring[colorIndex]);
  ...
  it("CY_10 - Ability to select any interior", () => {
    ...
    $PageObject.selectVariant(modelCode,variantCode);
    $PageObject.clickInteriorTab();
    $PageObject.selectInterior(interiorZeroAmountIndex,variantInteriorOptions[interiorZeroAmountIndex]);
    $PageObject.validateCarImgSrc(outbackPageElements.imgSrc.awdTouring[0]);
    $PageObject.clickShowFullSummary();
